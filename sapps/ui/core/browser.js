(function($core) {
    ...
    var LocationHistory = function(baseUrl, callback){
        console.log('LocationHistory(base=' + baseUrl + ')');
    ...
    var LocationHash = function(baseUrl, callback){
        console.log('LocationHash(base=' + baseUrl + ')');
        
        this.prepare = function(url){
            
        };
        this.set = function(url, replace){
            url = urlResolve(url).pathname;
            location.hash = url
            callback(url);
        };
        this.get = function(){
    ...
    $core.Browser = function(options){
        options = sjs.extend({}, {
            html5:isHistory
        },options || {});
        var baseUrl = getBaseUrl(options.base || location.href);
        var Location = options.html5 ? LocationHistory : LocationHash;
